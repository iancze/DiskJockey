...
@everywhere using JudithExcalibur.hmodel
...
function fprob(p::Vector{Float64})
    ...
    if config["fix_d"]
        ...
        M_star, r_c, T_10m, q_m, T_10a, q_a, gamma, h, delta, logM_gas, ksi, incl, PA, vel, mu_RA, mu_DEC = p
    else
    ...
    if ksi <= 0. || T_10m <= 0. || T_10a <= 0. || T_10a < T_10m || r_c <= 0.0 || M_star <= 0.0
        return -Inf
    end
...
if config["fix_d"]
    params = ["M_star", "r_c", "T_10m", "q_m", "T_10a", "q_a", "gamma", "h", "delta", "logM_gas", "ksi", "incl", "PA", "vel", "mu_RA", "mu_DEC"]
else
