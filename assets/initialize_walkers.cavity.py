...
# Generally, you want at least a few walkers for each dimension you may be exploring. For the **cavity** model, there are 13 parameters.
...
p0 = np.array([np.random.uniform(1.03, 1.05, nwalkers), # mass [M_sun]
              np.random.uniform(20., 21.0, nwalkers), #r_c [AU]
               np.random.uniform(1., 2.0, nwalkers), #r_cav [AU]
              np.random.uniform(110., 115, nwalkers), #T_10 [K]
              np.random.uniform(0.70, 0.71, nwalkers), # q 
              np.random.uniform(2.0, 3.0, nwalkers), # gamma_cav
              np.random.uniform(-3.4, -2.8, nwalkers), #log10 Sigma_c [log10 g/cm^2]
